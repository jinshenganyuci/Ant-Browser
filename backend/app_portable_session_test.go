...
func TestPortableSessionImportStagesCookiesBeforeFirstLaunch(t *testing.T) {
    ...
    for name, value := range entries {
        if err := writeProfilePackageJSON(zw, name, value); err != nil { t.Fatal(err) }
    }
    w, err := zw.Create("user-data/source/Default/Preferences")
    if err != nil { t.Fatal(err) }
    if _, err := w.Write([]byte(`{"session":{"restore_on_startup":1}}`)); err != nil { t.Fatal(err) }
    if err := zw.Close(); err != nil { t.Fatal(err) }
    if err := out.Close(); err != nil { t.Fatal(err) }
    ...
    if err != nil { t.Fatalf("跨电脑导入必须暂存独立 Cookie 登录态，在首次网页导航前恢复；当前仅还原原电脑加密文件: %v", err) }
    var got struct { Cookies []map[string]any `json:"cookies"` }
    if err := json.Unmarshal(data, &got); err != nil { t.Fatal(err) }
    if len(got.Cookies) != 1 || got.Cookies[0]["value"] != "fixture-only" { t.Fatal("Cookie 登录态未完整保留") }
}
